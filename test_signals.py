...
def main():
    ...
    try:
            ...
            if data is not None and len(data) > 0:
                ...
                if signal:
                    ...
                    if 'technical_score' in signal.reasoning:
                        print(f"   Technical Score: {signal.reasoning['technical_score']:.3f}")
                    if 'ml_score' in signal.reasoning:
                        print(f"   ML Score: {signal.reasoning['ml_score']:.3f}")
                    if 'combined_score' in signal.reasoning:
                        print(f"   Combined Score: {signal.reasoning['combined_score']:.3f}")
                ...
                print(f"   Individual generators: {len([s for s in all_signals.values() if s is not None])}/{len(all_signals)} active")
        ...
        for symbol in test_symbols:
            ...
            if metrics:
                print(f"   {symbol}: {metrics.get('total_signals', 0)} signals generated")

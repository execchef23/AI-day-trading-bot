...
from .base_signal import BaseSignalGenerator, TradingSignal, SignalType, SignalStrength
...
class MLTechnicalSignalGenerator(BaseSignalGenerator):
    ...
    def _determine_signal_type(self, combined_score: float) -> tuple[SignalType, SignalStrength]:
        ...
        if combined_score >= self.thresholds['strong_buy']:
            ...
            return SignalType.BUY, SignalStrength.STRONG if combined_score > 0.7 else SignalStrength.MODERATE
            ...
            return SignalType.SELL, SignalStrength.STRONG if combined_score < -0.7 else SignalStrength.MODERATE
        else:
    ...
    def _get_key_indicators(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        key_cols = ['rsi', 'macd', 'bb_position', 'volume_ratio', 'stoch_k', 'williams_r']
        for col in key_cols:

...
from .base_signal import SignalStrength, SignalType, TradingSignal
...
class SignalManager:
    ...
    def get_consensus_signal(
        ...
        valid_signals = {
            name: signal for name, signal in signals.items() if signal is not None
        }
    ...
    def _calculate_consensus(
        self, signals: Dict[str, TradingSignal], data: pd.DataFrame, symbol: str
    ) -> Optional[TradingSignal]:
        ...
        same_type_signals = [
            s for s in signals.values() if s.signal_type == most_common_type
        ]
        avg_confidence = sum(s.confidence for s in same_type_signals) / len(
            same_type_signals
        )
    ...
    def _store_signal_history(
        self, symbol: str, signals: Dict[str, Optional[TradingSignal]]
    ):
        ...
        entry = {"timestamp": datetime.now(), "signals": signals}
    ...
    def get_performance_metrics(self, symbol: str) -> Dict[str, Any]:
        ...
        for gen_name, confidences in metrics["average_confidence"].items():
            if confidences:
                metrics["average_confidence"][gen_name] = sum(confidences) / len(
                    confidences
                )
            else:
    ...
    def reset_history(self, symbol: Optional[str] = None):
        ...
        logger.info(
            f"Reset signal history for {'all symbols' if not symbol else symbol}"
        )

...
class DataPreprocessor:
    ...
    def validate_data(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        if not errors and len(df) > 0:
            invalid_prices = (df['High'] < df['Low']) | (df['High'] < df['Close']) | (df['Low'] > df['Close'])
            if invalid_prices.any():
                errors.append(f"Invalid price relationships found in {invalid_prices.sum()} rows")
    ...
    def prepare_for_training(self, df: pd.DataFrame, 
        ...
        try:
            ...
            logger.info(f"Using {len(clean_data)} samples with {len(feature_cols)} features")
            ...
            self.is_fitted = True
            logger.info(f"Data preparation complete. Train: {len(train_data)}, Val: {len(val_data)}, Test: {len(test_data)}")
    ...
    def get_feature_summary(self, df: pd.DataFrame) -> Dict:
        ...
        feature_cols = [col for col in self.feature_engineer.feature_columns if col in df.columns]
        ...
        return {
            ...
            'feature_types': {
                'price_features': len([col for col in feature_cols if any(x in col for x in ['return', 'ratio', 'gap'])]),
                'technical_indicators': len([col for col in feature_cols if any(x in col for x in ['rsi', 'macd', 'sma', 'ema', 'bb_'])]),
                'volume_features': len([col for col in feature_cols if 'volume' in col or 'obv' in col or 'vpt' in col]),
                'volatility_features': len([col for col in feature_cols if 'volatility' in col or 'atr' in col]),
                'time_features': len([col for col in feature_cols if any(x in col for x in ['day_', 'month', 'quarter', 'year'])]),
                'lag_features': len([col for col in feature_cols if '_lag_' in col]),
                'rolling_features': len([col for col in feature_cols if any(x in col for x in ['_mean_', '_std_', '_min_', '_max_', '_median_'])])
            }
        }
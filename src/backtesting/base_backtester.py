...
class BacktestResults:
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        if losing_trades and winning_trades:
            metrics["profit_factor"] = abs(metrics["average_win"]) / abs(
        ...
        # Duration metrics
        durations = [
            t.duration.total_seconds() / 3600 for t in closed_trades if t.duration
        ]  # hours
        if durations:
    ...
    def print_summary(self):
        ...
        if "error" not in self.metrics:
            ...
            if metrics.get("average_trade_duration_hours"):
                print(
                    f"   Avg Trade Duration: {metrics['average_trade_duration_hours']:.1f} hours"
                )
...
class BaseBacktester(ABC):
    ...
    def __init__(
        self, initial_capital: float = 100000, commission: float = 1.0, **params
    ):
    ...
    def _execute_trade(
        ...
        if trade_type == "BUY":
            if portfolio.cash < cost:
                self.logger.warning(
                    f"Insufficient cash for {symbol} trade: need ${cost:.2f}, have ${portfolio.cash:.2f}"
                )
                return None
        ...
        self.logger.debug(
            f"Executed {trade_type} trade: {quantity} shares of {symbol} at ${price:.2f}"
        )
        return trade
    ...
    def _close_trade(
        ...
        self.logger.debug(
            f"Closed trade: {trade.symbol} PnL=${trade.pnl:.2f} ({reason})"
        )

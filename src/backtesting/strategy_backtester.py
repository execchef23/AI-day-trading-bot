...
class SignalBasedBacktester(BaseBacktester):
    ...
    def run_backtest(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"Backtesting from {all_dates[0]} to {all_dates[-1]} ({len(all_dates)} days)"
        )
        ...
        self.logger.info(
            f"Backtest completed. Portfolio value: ${portfolio.total_value:.2f}"
        )
    ...
    def _find_new_opportunities(
        ...
        for symbol, symbol_data in data.items():
            ...
            # Generate signal
            try:
                signal = self.signal_manager.get_consensus_signal(
                    historical_data, symbol
                )

                if signal and self._should_take_signal(signal, portfolio):
                    self._execute_signal_trade(
                        portfolio, signal, current_date, historical_data
                    )
    ...
    def _execute_signal_trade(
        ...
        if signal.signal_type in [SignalType.BUY, SignalType.STRONG_BUY]:
            ...
            if trade:
                ...
                self.logger.info(
                    f"BUY signal executed: {quantity} shares of {symbol} at ${current_price:.2f}"
                )
            ...
            if current_position > 0:
                ...
                if existing_trade and existing_trade.is_open:
                    ...
                    self.logger.info(
                        f"SELL signal executed: closed position in {symbol} at ${current_price:.2f}"
                    )

    def _close_all_positions(
        self, portfolio: Portfolio, data: Dict[str, pd.DataFrame], final_date: datetime
    ):
        ...
        for trade in open_trades:
            ...
            if symbol in data and final_date in data[symbol].index:
                ...
                self._close_trade(
                    portfolio, trade, final_price, final_date, "END_OF_BACKTEST"
                )
            else:
        ...
        self.logger.info(
            f"Closed {len(open_trades)} remaining positions at end of backtest"
        )

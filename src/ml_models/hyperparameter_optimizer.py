...
class HyperparameterOptimizer:
    ...
    def optimize_model(
        ...
        if model_type == "xgboost":
            objective_func = lambda trial: self.objective_xgboost(
                trial, X_train, y_train, X_val, y_val
            )
        elif model_type == "lightgbm":
            objective_func = lambda trial: self.objective_lightgbm(
                trial, X_train, y_train, X_val, y_val
            )
        else:
        ...
        logger.info(
            f"Optimization complete for {model_type}. Best RMSE: {study.best_value:.4f}"
        )
    ...
    def optimize_all_models(
        ...
        # Optimize XGBoost
        results["xgboost"] = self.optimize_model(
            "xgboost", X_train, y_train, X_val, y_val
        )

        # Optimize LightGBM
        results["lightgbm"] = self.optimize_model(
    ...
    def save_optimization_results(
        self, results: Dict[str, Dict[str, Any]], filename: str = None
    ) -> None:
...
def run_hyperparameter_optimization(
    X_train, y_train, X_val, y_val, n_trials: int = 50, timeout: int = 300
) -> Dict[str, Dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    results = run_hyperparameter_optimization(
        X_train, y_train, X_val, y_val, n_trials=5, timeout=60
    )

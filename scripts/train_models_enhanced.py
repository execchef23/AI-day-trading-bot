...
def load_or_fetch_data():
    ...
    if os.path.exists(processed_data_dir):
        ...
        if csv_files:
            ...
            latest_file = max(csv_files, key=lambda x: os.path.getctime(os.path.join(processed_data_dir, x)))
...
def main():
    ...
    try:
        ...
        if choice == "2":
            ...
            optimizer = HyperparameterOptimizer(n_trials=20, timeout=300)  # 5 min per model
        ...
        trainer.show_sample_predictions(
            training_data['X_test'].tail(), 
            training_data['y_test'].tail()
        )
        ...
        print(f"   Direction Accuracy: {ensemble_performance['direction_accuracy']:.4f}")
        ...
        summary_path = os.path.join("models", f"training_summary_enhanced_{symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
